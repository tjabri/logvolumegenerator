...
pub fn create_log_entry(s: &String) -> String {
    format!("{} - INFO - {}", chrono::Utc::now().to_rfc3339(),  s.as_str())
}

pub fn calculate_iteration_pause(total_output_bytes: u64, total_output_duration_secs: u64, entry_size_bytes: usize) -> Result<std::time::Duration, RuntimeError> {
    if total_output_bytes == 0 || total_output_duration_secs == 0 || entry_size_bytes == 0 {
        return Err(RuntimeError::InvalidArgument);
    }
    let lines_per_sec = (total_output_bytes as f64 / entry_size_bytes as f64) / total_output_duration_secs as f64; 
